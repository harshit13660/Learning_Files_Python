...
def Main_Jarvis_Speak():
    if termin==0:
        ...
        def awake():
            ...
            while jag != 'wake up' or check_start!=1:
                try:
                    with mic1 as so:
                ...
                if query != None:
                    ...
                    if 'who are you' in query:
                        ...
                        with open('Programs_Dict.json','r') as pd:
                            program_data=eval(pd.read())
                            for p_data in program_data:
                                if query.split('open ')[1].replace(" ","")==p_data:
                                    speaker("ok")
                                    ...
                                    def top_asking():
                                        ...
                                        top_win.geometry("500x100+433+234")
                                        top_win.config(background='#0a0a0a')
                                        top_Label = Label(top_win, text='How Would You Like To Call It?',bg='#0a0a0a',fg='white',font="Ethnocentric 10 bold").pack(pady=5)
                                        top_Entry = Entry(top_win, width=80,bg='white',fg="black",font='Arial 15 bold',justify='center')
                                        top_Entry.pack()
                                        top_asking_button=Button(top_win,text="Ok",command=lambda:user_entry(top_Entry.get(),top_win),font="Ethnocentric 12 bold").pack(pady=4)
                        ...
                        def check_for_driver():
                            ...
                            def check_for_test_driver():
                                global driver
                                speaker('Wait a minute Checking for drivers')
                                try:
                        ...
                        def add_web_test():
                            ...
                            def get_url_auto(url_entry,auto_test_URL_window,name_of_file):
                                global url_for_xp_file
                                if len(url_entry)==0:
                                    speaker("please enter valid url")
                                else:
                        ...
                        def start_web_test(perform_querry):
                            ...
                            if perform_querry in dict_read_auto:
                                ...
                                speaker("Your page is successfully Loaded")

                        driver_ack=check_for_driver()
                        if(driver_ack==0):
                            perform_querry=query.split(" ",1)[1]
                            print(perform_querry)
                            with open("x_path.json",'r') as f:
                                x_path_data=f.read()
